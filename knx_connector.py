...
class KNXConnector(Connector, Thread):
    ...
    def __dpt9001_bytes(value):
        # Encode a temperature (°C) into 2-byte KNX DPT 9.001
    ...
    def __do_group_write(self, group_address, value, data_type):
        # Special handling: 4-mode HVAC user setpoint (DPT 275.100, 8 bytes).
        # Take one temperature from ThingsBoard and fill all 4 modes with it.
        if data_type in ('setpoint_heat_user', 'setpoint4', 'setpoint_4_modes'):
            t = float(value)
            payload = (self.__dpt9001_bytes(t) + self.__dpt9001_bytes(t)
                       + self.__dpt9001_bytes(t) + self.__dpt9001_bytes(t))
            self.__log.info('Writing 4-mode setpoint (DPT 275.100) to %s = %s -> %r',
                            group_address, t, payload)
            group_value_write(self.__client, group_address, payload)
        else:
    ...
    def __process_attribute_update(self, group_address, data_type, value):
        try:
            if self.__client.connection_manager.connected:
                self.__do_group_write(group_address, value, data_type)
